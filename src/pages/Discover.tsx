...
import { AddressSearch } from '@/components/AddressSearch'; // Ensure this path matches where you created the file
...
const Discover = () => {
  ...
  const [selectedDistance, setSelectedDistance] = useState<number | null>(5); // Default 5km

  // --- 2. Fetch Data ---
  useEffect(() => {
    // If we have a location (real or manual), fetch food.
    // If not, try to request it automatically.
    if (location) {
      fetchFood();
    } else {
      // If auto-location fails, fetchFood will still run below due to the fallback logic
      fetchFood();
      if (!locationLoading) requestLocation();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [location, searchTerm, selectedDistance]); // Re-run when these change
  ...
  const fetchFood = async () => {
    try {
      ...
      // 1. Use Real/Searched Location if available.
      // 2. Fallback to "Test Location" (Arisha's) if browser location fails (for Laptop Dev).
      ...
      // Debug Log
      console.log("📍 Searching Food From:", { 
        source: location?.latitude ? "Real/Manual" : "Dev Fallback",
        lat: userLat, 
        long: userLong, 
        radius: radiusVal 
      });
      ...
      const { data, error } = await supabase
        .rpc('search_food', { 
          lat: userLat,
          long: userLong,
          radius_meters: Number(radiusVal), 
  ...
  const toggleLike = async (e: React.MouseEvent, foodId: string) => {
    ...
    try {
      ...
      fetchUserFavorites(); // Revert on error
    }
  };
